...
- **[LLM 인퍼런스 병목 및 최적화 방안](./inference-optimization.md)**: Prefill/Decode 단계의 차이점, PagedAttention, Continuous Batching, 양자화 등의 핵심 최적화 개념.
- **[LLM API 및 서빙 제공자 종합 가이드](./llm-api-providers.md)**: 다양한 상용 API 제공자(CSP, Groq 등) 계층도 및 자체 호스팅(vLLM, Ollama) 옵션 정리.
