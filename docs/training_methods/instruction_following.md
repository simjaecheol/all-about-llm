...
## 지시사항 학습의 단계와 발전 (2026)

현대적인 LLM 학습에서 지시사항 따르기 능력은 단순히 SFT(지도 미세 조정)만으로 완성되지 않으며, 강화학습과의 밀접한 결합이 핵심입니다.

1. **SFT (Supervised Fine-Tuning)**: 인간이 작성하거나 고성능 모델이 생성한 고품질 [지시사항, 응답] 쌍 데이터셋을 사용하여 모델이 지시를 따르는 기본적인 태도를 학습시킵니다.
2. **Alignment & Preference (정렬)**: SFT 이후, 인간과 AI의 선호도에 더 정밀하게 맞추기 위해 **DPO, KTO, Online DPO** 등의 기법을 적용합니다. 특히 **KTO**는 비교 데이터 없이도 효율적인 정렬을 가능하게 하며, **Online DPO**는 모델의 실시간 출력을 반영하여 성능을 극대화합니다.
3. **Reasoning Integration (추론 통합)**: 지시사항 이행 시 단순히 답을 내놓는 것이 아니라, 내부적인 '사고 과정(Chain of Thought)'을 명시적으로 포함하도록 학습시켜 복잡한 명령도 논리적인 근거를 바탕으로 처리할 수 있게 합니다.
4. **Self-Correction (자가 수정)**: 모델이 지시사항을 이행한 후 스스로 결과를 검토하고 수정하는 능력을 학습에 포함하여, 지시사항 준수율을 비약적으로 향상시킵니다.
